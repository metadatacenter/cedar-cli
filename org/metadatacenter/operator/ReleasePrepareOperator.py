...
class ReleasePrepareOperator(Operator):
    ...
    def expand(task: PlanTask):
        ReleasePrepareOperator.expand_for_release(task, task.parameters['branch_type'])
    ...
    def expand_for_release(cls, task, branch_type: PreReleaseBranchType):
        ...
        for repo in repo_list:
            if repo.repo_type == RepoType.JAVA_WRAPPER:
                ...
                shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_java(repo, branch_type))
                ...
                if repo.pre_post_type == PrePostType.SUB:
                    ...
                    shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_angular_src_sub(repo, branch_type))
                    task.add_task_as_task(shell_wrapper)
                    ...
                    shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_angular_src(repo, branch_type))
                    task.add_task_as_task(shell_wrapper)
                ...
                if repo.pre_post_type == PrePostType.SUB:
                    ...
                    shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_angular_dist_sub(repo, branch_type))
                    task.add_task_as_task(shell_wrapper)
                elif repo.pre_post_type == PrePostType.NONE:
                    shell_wrapper = PlanTask("Prepare release of angular dist standalone project", TaskType.SHELL_WRAPPER, repo)
                    shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_angular_dist(repo, branch_type))
                    task.add_task_as_task(shell_wrapper)
                ...
                shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_angular_js(repo, branch_type))
                task.add_task_as_task(shell_wrapper)
                ...
                if repo.pre_post_type == PrePostType.PRE:
                    ...
                    shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_multi_pre(repo, branch_type))
                    task.add_task_as_task(shell_wrapper)
                elif repo.pre_post_type == PrePostType.POST:
                    shell_wrapper = PlanTask("Wrap up release multi project", TaskType.SHELL_WRAPPER, repo)
                    shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_multi_post(repo, branch_type))
                    task.add_task_as_task(shell_wrapper)
            elif repo.repo_type == RepoType.MKDOCS:
                shell_wrapper = PlanTask("Prepare release of mkdocs repo", TaskType.SHELL_WRAPPER, repo)
                shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_plain(repo, branch_type))
                task.add_task_as_task(shell_wrapper)
                ...
                if repo.pre_post_type == PrePostType.SUB:
                    ...
                    shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_plain_sub(repo, branch_type))
                    task.add_task_as_task(shell_wrapper)
                ...
                shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_development(repo, branch_type))
                task.add_task_as_task(shell_wrapper)
                ...
                shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_docker_deploy(repo, branch_type))
                task.add_task_as_task(shell_wrapper)
                ...
                shell_wrapper.add_task_as_task(ReleasePrepareShellTaskFactory.prepare_docker_build(repo, branch_type))
                task.add_task_as_task(shell_wrapper)
            else:
