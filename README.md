...
- `cli.sh` is the shell wrapper used by the `cedarcli` alias. It activates the repository virtual
  environment, preserves the caller's working directory for `build this` and `publish this`, and
  returns the Python process's exit status.
