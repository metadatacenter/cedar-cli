    ...
    def expand(self, task: PlanTask):
        ...
        for repo in repo_list_flat:
            if repo.repo_type == RepoType.JAVA_WRAPPER:
                ...
                shell_wrapper.add_task_as_task(ShellTaskFactory.maven_deploy_skip_tests(repo))
                ...
                shell_wrapper.add_task_as_task(ShellTaskFactory.npm_install_legacy_ng_build_publish(repo))
                task.add_task_as_task(shell_wrapper)
            elif repo.repo_type == RepoType.ANGULAR_JS:
                shell_wrapper = PlanTask("Deploy angularJS project", TaskType.SHELL_WRAPPER, repo)
                shell_wrapper.add_task_as_task(ShellTaskFactory.npm_install_publish(repo))
                task.add_task_as_task(shell_wrapper)
            else:
