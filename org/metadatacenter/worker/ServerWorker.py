...
class ServerWorker(Worker):
    ...
    def status():
        ...
        table = Table("Server", "Status", "Port", 'Error', title="CEDAR native host and infrastructure status")
        ...
        for server in Util.get_servers():
            ...
            #     table.add_section()
        console.print(table)
        console.print("[dim]For container status, run: cedarcli docker status[/dim]")
