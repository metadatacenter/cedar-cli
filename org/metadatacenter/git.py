import typer
from typing import List

from org.metadatacenter import git_clone, git_list
from org.metadatacenter.worker.GitWorker import GitWorker
from org.metadatacenter.util.CliResult import exit_on_failure

app = typer.Typer(no_args_is_help=True)
app.add_typer(git_clone.app, name="clone")
app.add_typer(git_list.app, name="list")

git_worker = GitWorker()


@app.command("status")
def status():
    exit_on_failure(git_worker.status())


@app.command("branch")
def git_1branch():
    exit_on_failure(git_worker.branch())


@app.command("pull")
def pull():
    exit_on_failure(git_worker.pull())


@app.command("fetch")
def pull():
    exit_on_failure(git_worker.fetch())


@app.command("remote")
def remote():
    exit_on_failure(git_worker.remote())


@app.command("checkout")
def checkout(branch: str):
    exit_on_failure(git_worker.checkout(branch))


@app.command("next")
def git_next():
    exit_on_failure(git_worker.next())


@app.command("add-commit-push")
def git_add_commit_push(
        comment: str,
        repo: str = typer.Option(..., "--repo", "-r", help="Exact repository name"),
        paths: List[str] = typer.Option(..., "--path", "-p", help="Explicit file or directory to stage")):
    try:
        result = git_worker.git_add_commit_push(comment, repo, paths)
    except ValueError as e:
        raise typer.BadParameter(str(e)) from e
    exit_on_failure(result)
